...
function ShowCivicCompletedPopup(player: number, civic:number, isCanceled:boolean)
    ...
    if civicInfo ~= nil then
        ...
        local TextMsg = Locale.Lookup("LOC_CIVICS_KEY_COMPLETED") .. Locale.ToUpper(Locale.Lookup(civicInfo.Name));
...
function ShowTechCompletedPopup(player: number, techId:number, isCanceled:boolean)
    ...
    if techInfo ~= nil then
        ...
        local TextMsg = Locale.Lookup("LOC_RESEARCH_CHOOSER_JUST_COMPLETED") .. Locale.ToUpper(Locale.Lookup(techInfo.Name));
        local pInstance :table = kTypeEntry.InstanceManager:GetInstance();
...
function ShowTechBoost(techIndex, iTechProgress, eSource)
    ...
    local TextMsg = Locale.Lookup("LOC_HUD_POPUP_TECH_BOOST_UNLOCKED") .. Locale.ToUpper(techName);
    local pInstance :table = kTypeEntry.InstanceManager:GetInstance();
...
function ShowCivicBoost(civicIndex, iCivicProgress, eSource)
    ...
    local TextMsg = Locale.Lookup("LOC_HUD_POPUP_CIVIC_BOOST_UNLOCKED") .. Locale.ToUpper(civicName);
    local pInstance :table = kTypeEntry.InstanceManager:GetInstance();
