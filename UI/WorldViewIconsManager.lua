...
function SetResourceIcon( pInstance:table, pPlot, type, state)
  ...
  if (pPlot and resourceInfo ~= nil) then
    ...
    if (textureSheet ~= nil) then
      ...
      --TQUI: Resource icon becomes transparent after being improved
      local TQUI_plotWasImproved;
      local TQUI_ICON_LOW_OPACITY :number = 0x77ffffff;
      local TQUI_optimalTileImprovement = resourceInfo.ImprovementCollection[1].ImprovementType; --Represents the tile improvement that utilizes the resource most effectively
      local TQUI_tileImprovement = GameInfo.Improvements[pPlot:GetImprovementType()]; --Can be nil if there is no tile improvement
      if (TQUI_tileImprovement ~= nil) then TQUI_tileImprovement = TQUI_tileImprovement.ImprovementType; end --If the tile improvement isn't nil, find the ImprovementType value
      if (TQUI_tileImprovement ~= TQUI_optimalTileImprovement) then
        TQUI_plotWasImproved = false;
      else
        TQUI_plotWasImproved = true;
      end
      if TQUI_plotWasImproved then
        pInstance.ResourceIcon:SetColor(TQUI_ICON_LOW_OPACITY);
      else
