...
function ViewMain(data: table)
    ...
    realFoodPerTurnToolTip = realFoodPerTurnToolTip .. "[NEWLINE][NEWLINE]" .. toPlusMinusString(-(data.FoodPerTurn - data.FoodSurplus)) .. Locale.Lookup("LOC_HUD_CITY_FOOD_CONSUMPTION")
    ...
    if ((table.count(data.Religions) > 0) or (data.PantheonBelief > -1)) then
        local ReligionTooltip :string = Locale.Lookup("LOC_HUD_CITY_YOUR_PANTHEON_BELIEF") .. "[NEWLINE][NEWLINE]";
        local noPantheon = true
        if ((table.count(data.Religions) > 0) and (data.PantheonBelief > -1)) then
        else
            ReligionTooltip = Locale.Lookup("LOC_HUD_CITY_RELIGION_NO_CITIZENS")
        end
        ...
        if (table.count(data.Religions) > 0) then
            ...
            for _, religion in ipairs(data.Religions) do
                ...
                if religion == data.Religions[DATA_DOMINANT_RELIGION] then
                    ReligionTooltip = ReligionTooltip .. "[NEWLINE]---------------------[NEWLINE]" .. religionName .. ": " .. religion.Followers .. Locale.Lookup("LOC_UI_RELIGION_FOLLOWERS");
                else
                    if (religion.ID > -1 and religion.Followers > 0) then
                        religiousMinoritiesExist = true;
                        religiousMinorities = religiousMinorities .. "[NEWLINE]" .. religionName .. ": " .. religion.Followers .. Locale.Lookup("LOC_UI_RELIGION_FOLLOWERS");
                    end
                end
            end
            ...
            if religiousMinoritiesExist then
                ReligionTooltip = ReligionTooltip .. "[NEWLINE]---------------------[NEWLINE]其他宗教信仰[NEWLINE]" .. religiousMinorities;
            end
        end
        Controls.ReligionGrid:SetToolTipString(ReligionTooltip);
    else
        Controls.ReligionGrid:SetToolTipString("该城市无宗教信仰.")
    end


    Controls.BreakdownNum:SetText(data.DistrictsNum .. "/" .. data.DistrictsPossibleNum);
    Controls.BreakdownLabel:SetText(Locale.Lookup("LOC_PEDIA_DISTRICTS_TITLE"));

    local DistrictTooltip = "";
    local CityCenter = true;
    for _, district in ipairs(data.BuildingsAndDistricts) do
        if district.isBuilt then
            --district is pillaged tooltipstext ;
            --print("CityPanel:district.isPillaged",district.isPillaged);
    ...
    repeatAvoidAddNew("LOC_HUD_CITY_REQUIRED_AMENITIES", "AmenitiesRequiredNum", true, true);
    ...
    for _, aTable in pairs(tableChanges["Neg"]) do
        HappinessTooltipString = HappinessTooltipString .. "[NEWLINE]-" .. aTable.Amenities .. " " .. aTable.AmenityType .. " ";
    end
    HappinessTooltipString = HappinessTooltipString .. "[NEWLINE]"
    for _, aTable in pairs(tableChanges["Pos"]) do
        HappinessTooltipString = HappinessTooltipString .. "[NEWLINE]+" .. aTable.Amenities .. " " .. aTable.AmenityType:sub(1, -4) .. " ";
    end
    ...
    local ProductionType = "NONE_YET";
    if (GameInfo.Buildings[productionHash]) then
        ProductionToolTip = ToolTipHelper.GetBuildingToolTip(productionHash, m_pPlayer:GetID(), selectedCity)
        ProductionType = GameInfo.Buildings[productionHash].BuildingType

    elseif (GameInfo.Units[productionHash]) then
        ProductionToolTip = ToolTipHelper.GetUnitToolTip(productionHash)
        ProductionType = GameInfo.Units[productionHash].UnitType

    elseif (GameInfo.Districts[productionHash]) then
        ProductionToolTip = ToolTipHelper.GetDistrictToolTip(productionHash)
        ProductionType = GameInfo.Districts[productionHash].DistrictType

    elseif (GameInfo.Projects[productionHash]) then
        ProductionToolTip = ToolTipHelper.GetProjectToolTip(productionHash)
        ProductionType = GameInfo.Projects[productionHash].Type

    else

        ProductionToolTip = Locale.ToUpper(Locale.Lookup("LOC_HUD_CITY_NOTHING_PRODUCED"))
    end
    ...
    elseif data.TurnsUntilGrowth >= 0 then
        ...
        GrowthTurnsBar_toolTip = Locale.ToUpper(Locale.Lookup("LOC_CITY_BANNER_TURNS_GROWTH", data.TurnsUntilGrowth))
    else
        Controls.GrowthNum:SetColorByName("StatBadCS");
        GrowthTurnsBar_toolTip = Locale.ToUpper(Locale.Lookup("LOC_CITY_BANNER_TURNS_STARVATION", math.abs(data.TurnsUntilGrowth)))
    end
    local cGrowth = selectedCity:GetGrowth();
    Controls.GrowthProgressString:SetText(Round(cGrowth:GetFood(), 1) .. "/" .. cGrowth:GetGrowthThreshold() .. " [ICON_FoodLarge]")
    ...
    if (data.CurrentProductionIcon) then
        --productionicon erclick to open civilopediascreen
        Controls.ProductionIcon:SetIcon(data.CurrentProductionIcon);
        Controls.ProductionIcon:SetToolTipString(ProductionToolTip)
        Controls.PortraitProductionButton:RegisterCallback(Mouse.eRClick, function() LuaEvents.OpenCivilopedia(ProductionType) end);
    ...
    if m_pCity ~= nil then
        ...
        if cityCulture ~= nil then
            ...
            if (newGrowthPlot ~= -1 and newGrowthPlot ~= m_GrowthPlot) then
                ...
                local currentNeeded :number = cityCulture:GetNextPlotCultureCost();
                ...
                if (Controls.PurchaseTileCheck:IsChecked()) then
                    ...
                    Controls.GrowthHexStack_Exact:SetHide(false)
                else
                    UILens.ClearHex(LensLayers.PURCHASE_PLOT, m_GrowthPlot);
                    UILens.SetLayerGrowthHex(LensLayers.PURCHASE_PLOT, Game.GetLocalPlayer(), m_GrowthPlot, 1, "GrowthHexBG");
                    Controls.GrowthHexStack_Exact:SetHide(true)
                end
                --GrowthHexBG, GrowthHexNext, GrowthHexCurrent
                ...
                Controls.TurnsLeftDescription_Exact:SetText("[ICON_CultureLarge] " .. Round(currentCulture, 1) .. "/" .. currentNeeded .. " (" .. toPlusMinusString(currentYield) .. ")");
                --Controls.TurnsLeftLabel_Exact:SetText("");
                Controls.GrowthHexStack_Exact:CalculateSize();
